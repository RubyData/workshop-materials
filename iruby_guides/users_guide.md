...
このリストから `Markdown` を選択するとセルは Markdown セルになり、セルの内容が Markdown として処理されるようになります。Markdown セルには、通常の Markdown テキストのほか、LaTeX 記法による数式を記述できます。インライン数式は`$`で囲み、ディスプレイ数式は`$$`で囲みます。数式は MathJax でレンダリングされます。
